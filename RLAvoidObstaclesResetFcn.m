...
%         map1=randi([1 6])
%             if map1<=2
% %                 in = setVariable(in,'mapName',mapMatrix=simpleMap);
%                 mapMatrix=simpleMap
%             elseif map1>=2 & map1<=4
% %                 in = setVariable(in,'mapName',mapMatrix=map5);
%                 mapMatrix=map5
%             else
% %                 in = setVariable(in,'mapName',mapMatrix=map6);
%                 mapMatrix=map6
%             end
%     end
   ...
   %     end        
        lidar = rangeSensor('HorizontalAngle', [0, 2*pi], 'HorizontalAngleResolution', 2*pi/179, 'RangeNoise', 0.01, 'Range', [0 maxRange]);
